...
<div class="min-h-screen fashion-gradient-light flex items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="max-w-md w-full relative z-10">
            ...
            <div class="inline-block p-1 fashion-gradient rounded-2xl shadow-lg float-animation">
                <div class="bg-white rounded-xl px-6 py-3">
                    <h1 class="text-2xl font-bold">
                        <span class="fashion-gradient bg-clip-text text-transparent">Verify OTP</span>
                    </h1>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8 relative overflow-hidden">
            ...
            <!-- Title -->
            <div class="text-center mb-6">
                <h2 class="text-2xl font-bold text-gray-900 mb-2">Verification Code</h2>
                <p class="text-gray-600 text-sm">
                    Enter the 6-digit code sent to
                </p>
                <p class="font-semibold text-lg mt-2 break-all" style="color: #a855f7;">
                    @if(session()->has('email'))
                        <i class="fas fa-envelope mr-2 text-sm" style="color: #ec4899;"></i>{{ session('email') }}
                    @else
                        <i class="fas fa-phone-alt mr-2 text-sm" style="color: #ec4899;"></i>{{ session('phone') }}
                    @endif
                </p>
            </div>
            ...
            <!-- OTP Form -->
            <form action="{{ route('web.register.verify-otp') }}" method="post" id="otpForm" class="space-y-6">
                @csrf
...
function resendOTP() {
    ...
    if (!resendBtn.disabled) {
        ...
        form.method = 'POST';
        form.action = '{{ route("web.register.resend-otp") }}';
