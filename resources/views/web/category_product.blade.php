...
<script>
    ...
    async function applyFilters() {
        ...
        console.log('Request URL:', `/category/${categorySlug}/filter?${params.toString()}`);
        ...
        try {
            ...
            if (data.success && productsContainer) {
                console.log('Products Count:', data.total);
                console.log('Showing Items:', data.firstItem, '-', data.lastItem);
                console.log('Products Data:', data.products);
                console.log('HTML Content Length:', data.html ? data.html.length : 'No HTML');
                
                // Log individual product details if available
                if (data.products && data.products.data) {
                    console.log('Product List:', data.products.data);
                    data.products.data.forEach((product, index) => {
                ...
                console.error('Controller returned error:', data);
                console.error('Error message:', data.message || 'No error message provided');
            }
        } catch (error) {
            console.error('Filter error:', error);
    ...
    // Attach product card click handlers
    function attachProductCardHandlers() {
        console.log('Attaching product card handlers...');
        document.querySelectorAll('.product-card').forEach(card => {
            card.addEventListener('click', function(e) {
                if (e.target.closest('button') || e.target.closest('a')) {
                    return;
...
<!-- <script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (collectionDropdownButton && collectionMenu && collectionChevron) {
            ...
            // Close dropdown when clicking outside
            document.addEventListener('click', function(e) {
                if (!collectionDropdownButton.contains(e.target) && !collectionMenu.contains(e.target)) {
                    collectionMenu.classList.add('hidden');
                    collectionChevron.style.transform = 'rotate(0deg)';
                }
            });
